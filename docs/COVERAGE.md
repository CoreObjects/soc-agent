...
| application | webshell | T1505.003 | ✅ 同 WAF 接入(上传类请求) | ⚠️ **WAF 侧取证就绪、结构校准**;主机侧三件套(WROTE/SPAWNED/回连)需被监控后端主机(容器化 DVWA=盲区)+ 上传类探针才能验 |
| ×4 | **generic_\<layer\>** | 该层兜底 | 该层任意告警 | ⚠️ 路由从未选中、未行使 |

## 验证进度小结

- **已真机验+校准(11):** kerberoast · lsass_dump · adcs · ingress_tool_transfer · suspicious_process · **registry_persistence** · **lateral_movement** · **c2_beacon** · **suspicious_outbound** · **dcsync** · **web_exploit**(靶场完善 B1-B4 + Phase C:良性发生器造真告警→取证验通,不造攻击 —— **Phase B 收口 + Phase C 主干通**)。kerberoast/lsass_dump 的 TP 通路已用真攻击走通;adcs 用 subject_dn 分诊;registry/lateral/network/dcsync/web 用良性活动(写 Run 键 / 远程 SYSVOL / 周期外连 / DC 正常复制 / WAF 探针)验取证。
- **未校准/受限(5):** webshell(WAF 侧取证就绪,主机侧三件套需被监控后端主机+上传探针,容器化 DVWA=盲区)+ 4 个 generic。
- **踩坑固化:** ① Wazuh 规则**别加 `win.system.channel` 字段**(会让 EID 规则不匹配)② 采集 `alert_min_level=7`,规则 **level≥7** 才进图 ③ 注册表 TargetObject 反斜杠单/双不一(alert 路径双转义)—— ingest `_parse_registry` 已丢空段归一 ④ **聚合边不存 `count`**(mapper 注释:现算)—— 周期性要 `count(e)` 数 Event,别读 `c.count` ⑤ EID3 端口 leaf 名是 **`dst_port`** 非 `dest_port` ⑥ `RESOLVES_TO` 在 cypher 声明了但**无 mapper 造边**(DNS 应答未映射)—— 查它每告警刷 warning,域反解/信誉是盲区 ⑦ **DCSync 4662 良性发生器**:光开 DS-Access 审计子类不够,域/config NC 根要加**复制权审计 SACL**;`repadmin /syncall` 用 **pull-only(`/Aed`)**,`/P` 推送方向撞 `8453`;单DC/域拓扑靠林级 config/schema 复制产良性 4662 ⑧ **WAF 接入**:ModSecurity=filebeat 直入(非 Wazuh,无 rule.mitre)→ technique 从 **CRS 规则号前缀**映(941→XSS/942→SQLi);WAF Event **无 ON_HOST**(TARGET Service/Uri 代替,跨层需 service→被监控 Host);ModSec v3 **无 transaction.unique_id** → event_uid 用稳定元组;runner 路由**按文档内容**(soc-app-* 混 waf+nginx)非索引名。
