...
| application | **web_exploit** | T1190 · T1059.007 | ✅ **WAF 接入建成**(C2 `_map_waf`:ModSecurity→Alert{source:waf},CRS 规则号→ATT&CK) | ✅ **真机验通**:取证正确(payload/命中CRS规则数组/**http_status+outcome 被拦否**/端点Uri/Service/源IP的WAF打击画像);跨层后端容器化=盲区 |
| application | **webshell** | T1505.003 | ✅ 自研规则 100808(EID11 写 web 脚本)+ Sysmon EID11 补 .aspx/.php 扩展 | ✅ **真机验通(host-side)**:取证正确(写入进程是否 w3wp/php-cgi/解码写入内容/是否 web 根/随后派生外连);verdict 从错判 malicious→正确 FP。srv02(castelblack)IIS 主机产 EID11 |
| ×4 | **generic_\<layer\>** | 该层兜底 | 该层任意告警 | ⚠️ 路由从未选中、未行使 |

## 验证进度小结

- **已真机验+校准(12):** kerberoast · lsass_dump · adcs · ingress_tool_transfer · suspicious_process · **registry_persistence** · **lateral_movement** · **c2_beacon** · **suspicious_outbound** · **dcsync** · **web_exploit** · **webshell**(靶场完善 B1-B5 + Phase C:良性发生器造真告警→取证验通,不造攻击 —— **12 个具体 skill 全验通,四层全覆盖**)。kerberoast/lsass_dump 的 TP 通路已用真攻击走通;adcs 用 subject_dn 分诊;其余用良性活动(写 Run 键 / 远程 SYSVOL / 周期外连 / DC 正常复制 / WAF 探针 / IIS 根写 .aspx)验取证。
- **未行使(4):** 4 个 generic 兜底(路由从未选中)。
- **踩坑固化:** ① Wazuh 规则**别加 `win.system.channel` 字段**(会让 EID 规则不匹配)② 采集 `alert_min_level=7`,规则 **level≥7** 才进图 ③ 注册表 TargetObject 反斜杠单/双不一(alert 路径双转义)—— ingest `_parse_registry` 已丢空段归一 ④ **聚合边不存 `count`**(mapper 注释:现算)—— 周期性要 `count(e)` 数 Event,别读 `c.count` ⑤ EID3 端口 leaf 名是 **`dst_port`** 非 `dest_port` ⑥ `RESOLVES_TO` 在 cypher 声明了但**无 mapper 造边**(DNS 应答未映射)—— 查它每告警刷 warning,域反解/信誉是盲区 ⑦ **DCSync 4662 良性发生器**:光开 DS-Access 审计子类不够,域/config NC 根要加**复制权审计 SACL**;`repadmin /syncall` 用 **pull-only(`/Aed`)**,`/P` 推送方向撞 `8453`;单DC/域拓扑靠林级 config/schema 复制产良性 4662 ⑧ **WAF 接入**:ModSecurity=filebeat 直入(非 Wazuh,无 rule.mitre)→ technique 从 **CRS 规则号前缀**映(941→XSS/942→SQLi);WAF Event **无 ON_HOST**(TARGET Service/Uri 代替,跨层需 service→被监控 Host);ModSec v3 **无 transaction.unique_id** → event_uid 用稳定元组;runner 路由**按文档内容**(soc-app-* 混 waf+nginx)非索引名 ⑨ **Webshell = host-triggered**(EID11 写 web 脚本,非 WAF):Sysmon EID11 include **默认没 web 脚本扩展**(有 .ps1/.bat/.hta 但漏 .aspx/.php/.jsp)→ 要往 `sysmonconfig-soc.xml` 加 + 热重载;判别核心是**写入进程**(w3wp/php-cgi=可疑 vs 部署/powershell=良性)+ 解码写入命令看内容;File 节点**双/单反斜杠重复**(同注册表 ③,alert 双转义 vs winlogbeat 单)—— recipe 侧 `replace('\\\\','\\')` 归一去重。GOAD IIS(srv02=castelblack)只静态站无上传页 → 良性发生器写 .aspx 的进程是部署式=FP 形状。
