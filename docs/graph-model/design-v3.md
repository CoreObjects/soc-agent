...
1. **图里数据无损** —— 凡进图的事件,每一次 occurrence 全字段保留;不聚合掉、不砍字段、不 offload 到 ES。
   > ★**实现合规现状(诚实标注,2026-07-21)**:**告警层已满足**——`:Alert.raw` 存整条原始告警,agent 不回 ES(满足#2)即拿全告警(payload/请求体/matched_var)。**事件层尚未满足**——`:Event` 当前只留取证相关标量(每类一个 leaf 子集),**字段级为有损投影**、非全字段,事件级 raw 全文**待补**。此前本稿多处把"无损"当已达成来写属**措辞失实**(下方"无损细节"等表述同此),以本注为准。
...
## 9. 体量与迁移
- **体量**:事件节点数 ≈ 入图事件数(此处"无损"=**条数/occurrence 不聚合**,非字段级——字段级现为 leaf 有损投影,见 §0 硬需求#1 合规注),靠**已建的 30 天留存窗口**扛,不靠丢数据/不靠回 ES。聚合边一对实体一条,不随频次增长。经验层(Verdict/Disposition)与身份实体一样**长期保留**(判例不随事件裁掉)。
