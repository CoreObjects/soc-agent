...
def collect(graph, alert, seed=None):
    ...
    b["actor_is_machine"] = bool(actor_sam and actor_sam.rstrip().endswith("$"))
    ...
    if actor_domain:                                  # 本域 DC 主机名(判"发起者是不是本域 DC")
        rows = graph.run_cypher(
            "MATCH (d:Domain {netbios:$nb}) RETURN d.fqdn AS domain_fqdn, d.dc AS dc_host", nb=actor_domain)
        ev["域DC"] = rows[0] if rows else {}

    ev["_图盲区"] = ("复制范围(是否含 krbtgt/全域,4662 不给被同步的目标账号)、授予复制权的前置 ACL 修改(5136,未接)、"
                    "DC/授权同步账号权威白名单 —— 未建模。判据:actor_is_machine 且 actor_sam 短名==本域 dc_host 短名 → DC 正常复制;"
                    "非DC账号(尤其用户)持复制权 → 成立")
    return ev
